...
const width = 500;
const leftColWidth = 180; // space reserved for language name + time text
const barAreaWidth = width - leftColWidth - 50; // 20px right padding
...
const svgHeader = `<?xml version="1.0" encoding="UTF-8"?>\n` +
  `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${contentHeight}" font-family="Fira Code, Courier New, monospace">\n` +
...
topLanguages.forEach((lang, i) => {
  ...
  const textX = 0;
  const nameX = textX;
  const timeX = 120; // adjust so time sits after name
